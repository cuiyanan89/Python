...
def map_fenge(height_up,height_down,width_left,width_right):
    print 'up',height_up,'down',height_down,'left',width_left,'right',width_right
...
#y为纵向分割线
    ...
    for i in xrange(width_left+1,width_right-width_left):
        map_list[(x)*62+i]='*'
        print 'i',i
    for j in xrange(height_up+1,height_down-height_up):
        map_list[y+(j)*62]='*'
        print 'j',j
    open_list = [0,1,1,1]
...
#        print open_list[i],
    ...
    for i in map_list:
        map_str += i
    print map_str
    print 'x',x,'y',y,'up',height_up,'down',height_down,'left',width_left,'right',width_right
